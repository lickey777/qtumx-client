...
<script>
  ...
  var defaultChainId = "mychain" + back;
  var defaultTokenName = "MYT" + back;
  var defaultDes = "my chain " + back;
  ...
  export default {
    data() {
      return {
        ...
        messageHeaderRules: [
          this.fieldRequired,
          v => /^[0-9a-f]{8}$/.test(v) || 'Message header is 4 bytes in hex format with "a-f" and "0-9",ignore "0x"',
        ],
      };
    },
    methods: {
      ...
      isPositiveInt(v) {
        return /^[0-9][0-9]*$/.test(v) || 'This field must be a positive integer';
      },
      portLimit(v) {
      ...
      async submit() {
        let halvingtime = 0;
        if(this.halvingInterval !== 0){
          halvingtime = this.halvingTimes;
        }
